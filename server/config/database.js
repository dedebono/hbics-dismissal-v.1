...
const initDatabase = (callback) => {
  db.serialize(() => {
    ...
    db.run(
      `CREATE TABLE IF NOT EXISTS dismissal_logs (
        id         INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    // ── Arrival display reset marker (keeps logs, clears only what UI shows) ──
    db.run(
      `CREATE TABLE IF NOT EXISTS arrival_display_resets (
